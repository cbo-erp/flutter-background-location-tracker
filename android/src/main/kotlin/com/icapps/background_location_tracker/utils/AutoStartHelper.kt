...
object AutoStartHelper {
    ...
    fun getAutoStartPermission(context: Context): Boolean {
        val buildInfo: String = Build.BRAND.lowercase()
        when (buildInfo) {
    ...
    private fun autoStartAsus(context: Context) {
        if (isPackageExists(context, PACKAGE_ASUS_MAIN)) {
            showAlert(context) { dialog: DialogInterface, which: Int ->
                try {
                    SharedPrefsUtil.saveAutoStart(context, true)
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            SharedPrefsUtil.saveAutoStart(context, true)
                            startIntent(
                                context, PACKAGE_OPPO_MAIN, PACKAGE_OPPO_COMPONENT_FALLBACK_A
                            )
                        } catch (exx: Exception) {
                            exx.printStackTrace()
                        }
    ...
    private fun autoStartVivo(context: Context) {
        if (isPackageExists(context, PACKAGE_VIVO_MAIN) || isPackageExists(
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            SharedPrefsUtil.saveAutoStart(context, true)
                            startIntent(
                                context, PACKAGE_VIVO_MAIN, PACKAGE_VIVO_COMPONENT_FALLBACK_A
                            )
                        } catch (exx: Exception) {
                            exx.printStackTrace()
                        }
            ...
            Logger.error("AutoStartHelper", "Vivo package not found")
        }
    }
