...
internal object SharedPrefsUtil {
    ...
    private const val KEY_CALBACK_HANDLER =
        "background.location.tracker.manager.CALLBACK_DISPATCHER_HANDLE_KEY"
    ...
    private const val KEY_TRACKING_INTERVAL =
        "background.location.tracker.manager.TRACKING_INTERVAL"
    private const val KEY_DISTANCE_FILTER = "background.location.tracker.manager.DISTANCE_FILTER"

    private const val KEY_NOTIFICATION_BODY =
        "background.location.tracker.manager.NOTIFICATION_BODY"
    private const val KEY_NOTIFICATION_ICON =
        "background.location.tracker.manager.NOTIFICATION_ICON"
    private const val KEY_NOTIFICATION_LOCATION_UPDATES_ENABLED =
        "background.location.tracker.manager.ENABLE_NOTIFICATION_LOCATION_UPDATES"
    private const val KEY_CANCEL_TRACKING_ACTION_TEXT =
        "background.location.tracker.manager.ENABLE_CANCEL_TRACKING_TEXT"
    private const val KEY_CANCEL_TRACKING_ACTION_ENABLED =
        "background.location.tracker.manager.ENABLE_CANCEL_TRACKING_ACTION"

    private const val PREF_KEY_APP_AUTO_START =
        "background.location.tracker.manager.PREF_KEY_APP_AUTO_START"

    private fun Context.prefs() = getSharedPreferences(SHARED_PREFS_FILE_NAME, Context.MODE_PRIVATE)


    fun saveAutoStart(ctx: Context, value: Boolean) {
        ctx.prefs().edit().putBoolean(PREF_KEY_APP_AUTO_START, value).apply()
    }
    fun getAutoStart(ctx: Context): Boolean = ctx.prefs().getBoolean(PREF_KEY_APP_AUTO_START, false)
    ...
    fun saveNotificationConfig(
        ...
        enableNotificationLocationUpdates: Boolean,
        enableCancelTrackingAction: Boolean
    ) {
        ctx.prefs()
            .edit()
            ...
            .putBoolean(
                KEY_NOTIFICATION_LOCATION_UPDATES_ENABLED,
    ...
    fun getNotificationBody(ctx: Context): String =
        ctx.prefs().getString(KEY_NOTIFICATION_BODY, "Background tracking active. Tap to open.")!!

    fun getNotificationIcon(ctx: Context): String? =
        ctx.prefs().getString(KEY_NOTIFICATION_ICON, null)

    fun getCancelTrackingActionText(ctx: Context): String =
        ctx.prefs().getString(KEY_CANCEL_TRACKING_ACTION_TEXT, "Stop Tracking")!!

    fun isNotificationLocationUpdatesEnabled(ctx: Context): Boolean =
        ctx.prefs().getBoolean(KEY_NOTIFICATION_LOCATION_UPDATES_ENABLED, false)

    fun isCancelTrackingActionEnabled(ctx: Context): Boolean =
        ctx.prefs().getBoolean(KEY_CANCEL_TRACKING_ACTION_ENABLED, false)
}